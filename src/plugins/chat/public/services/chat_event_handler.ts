...
export class ChatEventHandler {
  ...
  private async handleToolCallEnd(event: ToolCallEndEvent): Promise<void> {
    ...
    try {
      ...
      if (result.waitingForAgentResponse) {
        ...
        // sendToolResult will create the tool message and we'll add it to timeline after
  ...
  private async sendToolResultToAssistant(toolCallId: string, result: any): Promise<void> {
    try {
      // Get current timeline (should include user message and assistant's tool call)
      const messages = this.getTimeline();

      // sendToolResult creates the tool message and includes it in the request
      ...
      // Now add the tool message to timeline so UI shows it
      this.onTimelineUpdate((prev) => [...prev, toolMessage]);
